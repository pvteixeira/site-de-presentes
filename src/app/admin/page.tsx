...
export default function AdminPage() {
  ...
  const copyWhatsAppMessage = (name: string, username: string, pass: string, id: string) => {
    const text = `✨ *Convite Especial - Casamento Aline & Klécio* ✨\n\nOlá *${name}*!\nPreparamos uma Área Exclusiva dos Padrinhos em nosso site com todas as informações das vestimentas e recados especiais!\n\n🔗 Acesse em: https://site-de-presentes.vercel.app/padrinhos (ou no nosso site)\n👤 *Usuário:* ${username}\n🔑 *Senha:* ${pass}\n\nCom carinho,\nAline & Klécio 💍`;
      ...
      <div className="max-w-6xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
          <div>
            ...
            <p className="text-xs text-gray-500 font-sans mt-1">Gerenciamento de Lista de Presentes & Credenciais dos Padrinhos</p>
          </div>
          ...
          <div className="flex items-center gap-3">
            <Link 
              href="/padrinhos" 
              target="_blank"
              className="flex items-center gap-2 bg-white dark:bg-zinc-800 text-[var(--foreground)] border border-gray-200 dark:border-zinc-700 px-4 py-2 rounded-lg text-sm hover:border-[var(--color-wedding-gold)] transition-colors"
            >
              <ExternalLink size={16} /> Ver Área dos Padrinhos
            </Link>

            {activeTab === 'gifts' && (
              <button 
                onClick={() => setIsAdding(true)}
                className="flex items-center gap-2 bg-[var(--color-wedding-gold)] text-white px-4 py-2 rounded-lg text-sm hover:bg-[var(--color-wedding-gold)]/90 transition-colors shadow-sm"
              >
        ...
        {activeTab === 'gifts' ? (
          ...
          <div className="space-y-6">
            <div className="bg-white dark:bg-zinc-900 p-6 rounded-2xl border border-gray-100 dark:border-zinc-800 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
              <div>
                <h2 className="text-xl font-serif text-[var(--foreground)]">Contas & Senhas dos Padrinhos</h2>
            ...
            <div className="bg-white dark:bg-zinc-900 rounded-2xl shadow-sm border border-gray-100 dark:border-zinc-800 overflow-x-auto">
              <table className="w-full text-left border-collapse min-w-[650px]">
                <thead>
                  <tr className="bg-gray-50 dark:bg-zinc-800/50 border-b border-gray-100 dark:border-zinc-800">
                    <th className="p-4 font-medium text-gray-500">Padrinho(s) / Papel</th>
                    <th className="p-4 font-medium text-gray-500">Usuário</th>
                    <th className="p-4 font-medium text-gray-500">Senha</th>
                    <th className="p-4 font-medium text-gray-500 text-right">Enviar no WhatsApp</th>
                  </tr>
                </thead>
                <tbody>
                  {PADRINHOS_ACCOUNTS.map((acc) => (
                    <tr key={acc.id} className="border-b border-gray-100 dark:border-zinc-800 hover:bg-gray-50/50 dark:hover:bg-zinc-800/20">
                      <td className="p-4">
                        <p className="font-medium font-serif text-[var(--foreground)]">{acc.name}</p>
                        <span className="inline-block px-2 py-0.5 text-[10px] font-semibold rounded bg-[var(--color-wedding-gold)]/10 text-[var(--color-wedding-gold)] capitalize mt-1">
                      ...
                      <td className="p-4 font-mono text-sm font-bold text-[var(--color-wedding-gold)]">
                        {acc.password}
                      </td>
                      <td className="p-4 text-right">
                        <button
                          onClick={() => copyWhatsAppMessage(acc.name, acc.username, acc.password, acc.id)}
                          className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium bg-green-500/10 text-green-600 dark:text-green-400 hover:bg-green-500/20 transition-colors"
                        >
                          {copiedId === acc.id ? (
                            <>
